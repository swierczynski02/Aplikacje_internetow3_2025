...
<div class="container">
    ...
    <a href="{{ route('szczegolyZamowienia.create') }}" class="btn btn-primary mb-3">Dodaj Nowy Szczegół Zamówienia</a>
    ...
    <table class="table">
        ...
        <tbody>
            @foreach($szczegolyZamowienia as $szczegol)
                <tr>
                    <td>{{ $szczegol->zamowienie->id }}</td>
                    <td>{{ $szczegol->bilet_id}}</td>
                    <td>{{ $szczegol->ilosc }}</td>
                    <td>{{ $szczegol->cena }}</td>
                    <td>{{ $szczegol->cena_calkowita }}</td>
                    <td>
                    <a href="{{ route('szczegolyZamowienia.edit', $szczegol->id) }}" class="btn btn-warning btn-sm">Edytuj</a>
                        
                         <!-- Formularz do usuwania z potwierdzeniem -->
                            <form action="{{ route('szczegolyZamowienia.destroy', $szczegol->id) }}" method="POST" onsubmit="return confirm('Czy na pewno chcesz usunąć ten bilet?');" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
