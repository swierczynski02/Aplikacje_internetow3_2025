...
<div class="container">
    ...
    <a href="{{ route('zamowienia.create') }}" class="btn btn-primary mb-3">Dodaj Nowe Zamówienie</a>
    ...
    <table class="table">
        ...
        <tbody>
            @foreach($zamowienia as $zamowienie)
                <tr>
                    <td>{{ $zamowienie->klient_id }}</td>
                    <td>{{ $zamowienie->data_zamowienia }}</td>
                    <td>{{ $zamowienie->cena_calkowita }}</td>
                    <td>{{ $zamowienie->status }}</td>
                    <td>{{ $zamowienie->status_platnosci }}</td>
                    <td>
                        <a href="{{ route('zamowienia.edit', $zamowienie->id) }}" class="btn btn-warning btn-sm">Edytuj</a>
                        
                        <!-- Formularz do usuwania z potwierdzeniem -->
                            <form action="{{ route('zamowienia.destroy', $zamowienie->id) }}" method="POST" onsubmit="return confirm('Czy na pewno chcesz usunąć ten bilet?');" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
