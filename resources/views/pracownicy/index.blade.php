...
<div class="container">
    ...
    <a href="{{ route('pracownicy.create') }}" class="btn btn-primary mb-3">Dodaj Nowy Szczegół Zamówienia</a>
    ...
    <table class="table">
        ...
        <tbody>
            @foreach($pracownicy as $pracownik)
                <tr>
                    <td>{{ $pracownik->uzytkownik_id}}</td>
                    <td>{{ $pracownik->rola }}</td>
                    <td>{{ $pracownik->data_zatrudnienia }}</td>
                    <td>
                        <a href="{{ route('pracownicy.edit', $pracownik->id) }}" class="btn btn-warning btn-sm">Edytuj</a>
                        
                         <!-- Formularz do usuwania z potwierdzeniem -->
                         <form action="{{ route('pracownicy.destroy', $pracownik->id) }}" method="POST" onsubmit="return confirm('Czy na pewno chcesz usunąć ten bilet?');" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
