    ...
    <div class="container">
        ...
        <form action="{{ route('uzytkownicy.update', $uzytkownik->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="nazwa_uzytkownika" name="nazwa_uzytkownika" value="{{ old('nazwa_uzytkownika', $uzytkownik->nazwa_uzytkownika) }}" required>
            </div>

            <div class="mb-3">
                <label for="email" class="form-label">Email</label>
                <input type="email" class="form-control" id="email" name="email" value="{{ old('email', $uzytkownik->email) }}" required>
            </div>

            <div class="mb-3">
                <label for="rola" class="form-label">Rola</label>
                <select class="form-select" id="rola" name="rola" required>
                    <option value="administrator" {{ old('rola', $uzytkownik->rola) == 'administrator' ? 'selected' : '' }}>Administrator</option>
                    <option value="klient" {{ old('rola', $uzytkownik->rola) == 'klient' ? 'selected' : '' }}>Klient</option>
                    <option value="pracownik" {{ old('rola', $uzytkownik->rola) == 'pracownik' ? 'selected' : '' }}>Pracownik</option>
                </select>
            </div>
