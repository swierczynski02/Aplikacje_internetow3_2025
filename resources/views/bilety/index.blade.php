...
<script src="{{ asset('js/accessibility.js') }}"></script>
    <div class="container">
        ...
        <a href="{{ route('bilety.create') }}" class="btn btn-primary mb-3">Dodaj bilet</a>
            ...
            <tbody>
                @foreach($bilety as $bilet)
                    <tr>
                        ...
                        <td>{{ $bilet->cena }}</td>
                        <td>
                            <a href="{{ route('bilety.edit', $bilet->id) }}" class="btn btn-warning btn-sm">Edytuj</a>
                            
                            <!-- Formularz do usuwania z potwierdzeniem -->
                            <form action="{{ route('bilety.destroy', $bilet->id) }}" method="POST" onsubmit="return confirm('Czy na pewno chcesz usunąć ten bilet?');" style="display:inline-block;">
                                @csrf
                                @method('DELETE')
